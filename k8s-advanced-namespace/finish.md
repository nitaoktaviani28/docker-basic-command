...
Hal utama yang perlu diingat: nama resource harus unik dalam satu Namespace, tetapi nama yang sama dapat dipakai kembali pada Namespace lain. Pada lab ini, `nginx-app` digunakan di `dev` dan `prod`. Inilah dasar pemisahan environment dan workload dalam satu Kubernetes cluster.
